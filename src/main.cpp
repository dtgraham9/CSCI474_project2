...
int rand_gen(int prev_track, int edge){
    return 0;
}
...
int main(int argc, char *argv[]){
    ...
    for(int i = 1; i < rand_tracks; ++i){
        ...
        if(hold_rand <= 10 && hold_rand >= 1){
            ...
            if(rand() % 1 == 0){
                track_queue_wgt.push_back(rand_gen(track_queue_wgt.back, 0));
            }
            else{
                track_queue_wgt.push_back(rand_gen_for(track_queue_wgt.back, max_tracks-1));
            }
        }
